/*
This file was autogenerated from 00_load_data.ipynb
...
public func notebookToScript(fname: Path){
    ...
    let getCells: () -> [[String:Any]] = {
        do {
            let data = try Data(contentsOf: fname.url)
            let jsonData = try JSONSerialization.jsonObject(with: data, options: .allowFragments) as! [String: Any]
            return jsonData["cells"] as! [[String:Any]]
        } catch {
            print("Can't read the content of \(fname): \(error)")
            return []
        }
    }
...
"""
    do {
        ...
        try url.parent.mkdir(.p)
        try module.write(to: url, encoding: .utf8)
    } catch {
        print("Can't write to file \(url): \(error)")
    }
}
