...
public struct ConvLayer: Layer {
    ...
    @differentiable
    public func call(_ input: TF) -> TF { return bn(conv(input)) }
}
...
public extension SwitchableLayer {
    func call(_ input: Input) -> Input { return isOn ? forward(input) : input }
...
public struct XResNet: Layer {
    ...
    @differentiable
    public func call(_ inp: TF) -> TF { return linear(pool(blocks(maxPool(stem(inp))))) }
